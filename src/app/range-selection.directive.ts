...
export class RangeSelectionDirective implements OnDestroy, OnInit {
  ...
  constructor(
    private zone: NgZone,
    { nativeElement }: ElementRef<HTMLTableElement>
  ) {
  ...
  private initListeners() {
    const withCell = pipe(
      map((event: MouseEvent) => ({
        event,
        cell: (event.target as HTMLElement).closest<HTMLTableCellElement>(
          'th,td'
        ),
      })),
      filter(({ cell }) => !!cell)
  ...
  private handleOutsideClick() {
    fromEvent(document, 'mouseup')
      ...
      .subscribe((event: any) => {
        if (
          !this.selecting &&
          !this.table.contains(event.target as HTMLElement)
        ) {
          this.clearCells();
        }
      });
  ...
  private getCellsBetween(
    ...
    const boundaries = {
      ...
      right: Math.max(
        startCoords.column + start.colSpan - 1,
        endCoords.column + end.colSpan - 1
      ),
      bottom: Math.max(
        startCoords.row + start.rowSpan - 1,
        endCoords.row + end.rowSpan - 1
      ),
